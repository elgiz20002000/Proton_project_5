...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $(".report .count").countUp({
    time: "3000",
    delay: "1",
  });

  const scroll_sections = document.querySelectorAll(".scroll");
  ...
  window.addEventListener("scroll", (e) => {
    ...
    scroll_sections.forEach((item, index) => {
      const sectionHeight = item.offsetHeight;
      const sectionTop = item.offsetTop - 50;
      if (scrollY > sectionTop && scrollY <= sectionTop + sectionHeight) {
        document.querySelectorAll(".promo li")[index].classList.add("active");
      } else {
        document
          .querySelectorAll(".promo li")
